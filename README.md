...
8. 表紙作成とプレビュー作成の初期実行：
- apache権限でcomistream/code/make_image_preview.shを実行します。コンテンツ量によりますが結構時間がかかります。進捗は必要に応じてjournalctl -fなどでログを確認してください。
...
- デバイス間同期を利用するためにはヘッダでログインしてください。ゲストではデバイス間同期は利用できません。
