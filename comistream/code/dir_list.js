...
function linkhook(e) {
  ...
  // 音楽ファイルの場合
  if( e.target.href.match(/\.(mp3|m4a|aac|flac|aiff|aif|wav|wave|ogg|oga|wma)$/i ) ){
    e.preventDefault();
    // 音楽プレイヤーを開く
    var musicPlayerHref = "/cgi-bin/music_player.php?file=" + fileLink + "&mode=open";
    document.getElementById("history").innerHTML = "<a class=\"history_music\" href="+location.origin+musicPlayerHref+">"+e.target.innerText+"</a>"
...
function login() {
  ...
  if (loginIcon.className === "guest") {
    ...
    if (username && username.trim()) {
      document.cookie = "comistreamUser=" + encodeURIComponent(username.trim()) + "; path=/; SameSite=Strict";
      location.reload();
    }
  } else {
    // ログアウト処理
    if (confirm("ログアウトしますか？")) {
      document.cookie = "comistreamUser=; path=/; expires=Thu, 01 Jan 1970 00:00:00 GMT; SameSite=Strict";
      location.reload();
    }
  }
}
...
function getBookmark() {
  ...
  var requestUrl = cgiPath + "?mode=list&file="+pathName;
  ...
  try{
    ...
    console.error('getBookmark XMLHttpRequest error:', e);
    alert(e);
    return;
  }

  debugLog('DEBUG getBookmark response status:', xmlHttp.status);
  debugLog('DEBUG getBookmark response text length:', xmlHttp.responseText.length);
  if (xmlHttp.responseText) {
    debugLog('DEBUG getBookmark response text (first 200 chars):', xmlHttp.responseText.substring(0, 200));
  }

  lines = xmlHttp.responseText.split(/\n/);
  debugLog('DEBUG getBookmark found', lines.length, 'lines in response');
  ...
  for(j=0; j < lines.length; j++){
    line = lines[j].split("\t");
    if (line.length < 2) continue; // 空行や不正なデータをスキップ
    
    var elm = document.getElementById(line[0]);
    debugLog('DEBUG getBookmark line', j, '- filename:', line[0], 'element found:', !!elm);
    
    // 要素が見つからない場合は、類似するIDがないか調査
    if (!elm) {
      debugLog('DEBUG Searching for similar elements for filename:', line[0]);
      ...
      for (let searchIdx = 0; searchIdx < Math.min(allIndexColName.length, 10); searchIdx++) {
        ...
        const searchText = searchElm.firstChild ? searchElm.firstChild.textContent : 'no text';
        debugLog('DEBUG Element', searchIdx, '- id:', searchElm.id, 'text:', searchText);
        if (searchText === line[0]) {
          debugLog('DEBUG Found matching text but different ID! Setting correct ID...');
          searchElm.setAttribute('id', line[0]);
          searchElm.id = line[0];
    ...
    if( elm ){
      elementsFound++;
      debugLog('DEBUG Element structure check for:', line[0]);
      
      // 既読リストとIDがマッチする場合、現在ページと最終ページを比較★
      if( Number(line[1]) >0 ){
        if( Number(line[1]) < Number(line[2]) ){
          // 読みかけ
          debugLog('DEBUG Setting open icon for:', line[0]);
          // <a>要素と<img>要素を確実に取得
          var iconLinkElements = elm.previousSibling ? elm.previousSibling.getElementsByTagName('a') : [];
          var iconImgElement = null;
          
          if (iconLinkElements.length > 0) {
            var imgElements = iconLinkElements[0].getElementsByTagName('img');
          ...
          if (iconImgElement) {
            ...
            elementsUpdated++;
            debugLog('Successfully set open icon for:', line[0]);
          } else {
            console.error('ERROR: Cannot find icon img element for:', line[0]);
          }
        }else{
          // 読み終わった（最終ページ0に設定されている）
          debugLog('DEBUG Setting done icon for:', line[0]);
          // <a>要素と<img>要素を確実に取得
          var iconLinkElements = elm.previousSibling ? elm.previousSibling.getElementsByTagName('a') : [];
          var iconImgElement = null;
          ...
          if (iconImgElement) {
            ...
            elementsUpdated++;
            debugLog('Successfully set done icon for:', line[0]);
          } else {
            console.error('ERROR: Cannot find icon img element for:', line[0]);
          }
        }
      ...
      if( line[3] ){
        debugLog('DEBUG Setting favorite for:', line[0]);
        if (elm.previousSibling) {
          ...
          elementsUpdated++;
          debugLog('Successfully set favorite for:', line[0]);
        } else {
          console.error('ERROR: Cannot set favorite - no previousSibling for:', line[0]);
        }
      }
    } else if (line[0] && line[0].trim()) {
      // 要素が見つからない場合のデバッグ情報
      debugLog('DEBUG Element not found for filename:', line[0], '- Available IDs:', 
        Array.from(document.querySelectorAll('.indexcolname')).map(el => el.id).slice(0, 10));
    }
  }
  
  debugLog('DEBUG getBookmark completed - Elements found:', elementsFound, 'Elements updated:', elementsUpdated);
}
...
function getHistory(){
  ...
  debugLog('DEBUG getHistory response text length:', xmlHttp.responseText.length);
  
  if (xmlHttp.responseText) {
    debugLog('DEBUG getHistory response text (first 200 chars):', xmlHttp.responseText.substring(0, 200));
...
function search(){
  ...
  var favOnly = document.getElementById("favbutton").style.backgroundImage ? true : false;
  ...
  rows.forEach(function(row) {
    ...
    if (shouldShow && favOnly) {
      ...
      const hasStarBackground = iconCell && iconCell.style.backgroundImage && iconCell.style.backgroundImage.includes('staron.png');
  ...
  directories.forEach(function(dirLink) {
    ...
    if (customIcon) {
      ...
      if (imgElement) {
        ...
        debugLog('DEBUG Applied custom icon for directory:', dirName, '->', customIcon);
      }
    }
  });
}
...
function reinitializePreviewFeatures() {
  ...
  const colname = document.getElementsByClassName("indexcolname");
  debugLog('DEBUG reinitializePreviewFeatures found', colname.length, 'elements');
  
  // デバッグ：data-image属性が設定されているか確認
  for (let i = 2; i < Math.min(colname.length, 5); i++) { // 最初の数個だけチェック
        ...
        if((typeof imageSrc !== 'undefined' )&&(imageSrc.length > 0)){
          ...
          e.currentTarget._previewShowTimer = setTimeout(function() {
            ...
            modalImage.onload = function() {
              ...
              let previewWindowHeight = Math.round(812 * document.documentElement.clientWidth / 1734);
              ...
              if (e.clientY + previewWindowHeight + 200 > document.documentElement.clientHeight) {
                ...
                // 通常の下表示
                if (e.clientY + 200 + previewWindowHeight > document.documentElement.clientHeight) {
                  modal.style.top = (document.documentElement.clientHeight - previewWindowHeight) + "px";
                } else {
                  modal.style.top = (e.clientY + 200) + "px";
                }
              }
